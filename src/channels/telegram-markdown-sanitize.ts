...
export function sanitizeTelegramLegacyMarkdown(input: string): string {
  ...
  // Percent-encode MarkdownV2 entity markers (~ _ * `) inside bare URLs. The
  // adapter autolinks a bare URL into `[url](url)`, and its MarkdownV2
  // safe-boundary trimmer treats a lone ~/_/* inside the link destination as
  // an unbalanced entity marker and truncates the message there — Telegram
  // then rejects it ("can't parse entities: Can't find end of a URL"). Clients
  // decode the escapes, so the link still resolves.
  text = text.replace(/https?:\/\/[^\s<>()[\]]+/g, (url) =>
    url.replace(/~/g, '%7E').replace(/_/g, '%5F').replace(/\*/g, '%2A').replace(/`/g, '%60'),
  );
